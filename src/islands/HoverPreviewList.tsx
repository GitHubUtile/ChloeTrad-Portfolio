      ...
      <ul>
        {rows.map((row, i) => (
          <li key={row.slug} className="group">
            ...
            >
              <span className="col-span-2 lg:col-span-1 font-mono text-xs text-[var(--muted)]">{row.number}</span>
              <span className="col-span-10 lg:col-span-6 font-display text-2xl lg:text-3xl text-[var(--ink)] group-hover:text-[var(--accent)] transition-colors duration-300">
                {row.title}
              </span>
              <span className="col-span-8 lg:col-span-4 font-mono text-xs uppercase tracking-[0.15em] text-[var(--muted)]">
                {row.discipline}
              </span>
              <span className="col-span-4 lg:col-span-1 font-mono text-xs text-[var(--muted)] text-right">
                {row.year}
              </span>
            </a>
          </li>
